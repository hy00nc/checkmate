...
x_dist_dataset = strategy.experimental_distribute_dataset(x_train_new)
y_dist_dataset = strategy.experimental_distribute_dataset(y_train)


start_time = time.perf_counter()
#model.fit(x = x_train_new, y = y_train, batch_size = 128, epochs = 5, callbacks = [tboard_callback])
#model.fit(x = x_train_new, y = y_train, batch_size = 128, epochs = 5)
### multi gpu support
parallel_model = multi_gpu_model(model, gpus=3)
parallel_model.compile(optimizer=optimizer, loss=loss)
parallel_model.fit(x = x_train_new, y = y_train, batch_size = 128, epochs = 5)


end_time = time.perf_counter() - start_time
print("Total time: {0}".format(end_time))
